            ...
            <td>
                <c:choose>
                    <c:when test="${books.bookCounts!=0}">    <!--如果 -->
                        <button type="button" name="borrow" id="btn1" class="layui-btn"  onclick="borrowBook(${books.bookId},${books.bookCounts})">上架</button>
                    </c:when>
                    <c:otherwise>  <!--否则 -->
                        <button type="button" name="borrow" id="btn1" class="layui-btn  layui-btn-disabled"  >上架</button>
                    </c:otherwise>
                </c:choose>
                <c:choose>
                    <c:when test="${books.bookSum!=books.bookCounts}">    <!--如果 -->
                        <button type="button" name="return" id="btn2" class="layui-btn " onclick="returnBook(${books.bookId},${books.bookCounts})">下架</button>
                    </c:when>
                    <c:otherwise>  <!--否则 -->
                        <button type="button" name="return" id="btn2" class="layui-btn layui-btn-disabled" >下架</button>
                    </c:otherwise>
                </c:choose>
